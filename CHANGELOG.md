...
### Added
- **Timeout recommendations in research skill**: Added detailed guidance on appropriate timeouts for web search tasks
- **AGENTS.md**: New file documenting cache cleanup procedures and local plugin update instructions

### Documentation
- Research skill now warns that 2-minute timeouts are insufficient for web search tasks
- Added timeout table with recommended values for different task types
- Documented why web search + reasoning requires longer timeouts (network latency, source synthesis)
